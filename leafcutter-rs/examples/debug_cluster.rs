...
fn main() {
    ...
    eprintln!(
        "null value {:.8} conc {:?} beta {:?} conv {}",
        fnull.value, fnull.conc, fnull.beta, fnull.converged
    );
    eprintln!("--- null restart");
    let fnull2 = fit_model(&dn, &fnull.beta, &fnull.conc, &params.fit);
    eprintln!(
        "null restart value {:.8} (gain {:.3e})",
        fnull2.value,
        fnull2.value - fnull.value
    );
    ...
    if let Some(path) = args.get(4) {
        ...
        eprintln!(
            "--- external solution: rust objective {:.8} |g|={:.3e}",
            ll,
            g.iter().map(|x| x * x).sum::<f64>().sqrt()
        );
        let f = fit_model(&df, &beta, &conc, &params.fit);
        eprintln!(
            "full fit from external value {:.8} conc {:?} beta {:?}",
            f.value, f.conc, f.beta
        );
    }
}
